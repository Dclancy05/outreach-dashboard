...
export interface SaveResult {
  id?: string
  probe?: { ok: boolean; detail?: string; error?: string }
}
...
export function ApiKeyEditModal({ open, onOpenChange, initial, onSaved }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const probe = j?.probe as SaveResult["probe"]
      const savedId = (j?.data?.id as string | undefined) ?? initial?.id
      if (probe) {
        if (probe.ok) toast.success(`Connected — ${probe.detail || "live test passed"}`)
        else toast.error(`Saved but not connected: ${probe.error || "live test failed"}`)
      } else {
        toast.success(isEdit ? "Key updated" : "Key added")
      }
      onSaved({ id: savedId, probe })
      onOpenChange(false)
    } catch (e: unknown) {
