...
async function primeRepoExclude(anyWorktreePath: string): Promise<void> {
  ...
  try {
    ...
    const cur = existsSync(excludePath) ? fs.readFileSync(excludePath, "utf8") : ""
    if (!cur.includes("/.claude/CLAUDE.md")) {
      const sep = cur.endsWith("\n") || cur === "" ? "" : "\n"
      fs.writeFileSync(excludePath, cur + sep + "# Auto-generated by terminal-server (per-session identity briefing)\n/.claude/CLAUDE.md\n", "utf8")
