...
async function writeWorktreeClaudeMd(self: Session, others: Session[]): Promise<void> {
  ...
  try {
    ...
    // Ensure git ignores .claude/ for this worktree only — don't pollute the
    // repo's .gitignore. Idempotent (writes only if missing).
    const excludePath = join(self.worktreePath, ".git", "info", "exclude")
    try {
      const cur = existsSync(excludePath) ? require("node:fs").readFileSync(excludePath, "utf8") : ""
      if (!cur.includes("/.claude/")) {
        writeFileSync(excludePath, cur + (cur.endsWith("\n") || cur === "" ? "" : "\n") + "/.claude/\n", "utf8")
      }
    } catch { /* worktree may not exist yet, or .git/info missing — best effort */ }
  } catch (e) {
