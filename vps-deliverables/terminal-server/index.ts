...
function createSession(input: CreateInput): Session {
  ...
  try {
    ...
    // Phase 2: write the rich identity CLAUDE.md the moment the worktree
    // exists, so claude (which auto-loads .claude/CLAUDE.md on session start)
    // sees it before the user types their first message. The siblings watcher
    // will keep refreshing it every 30s.
    void writeWorktreeClaudeMd(session, Array.from(sessions.values()).filter((o) => o.id !== id))
...
// Phase 2 of the enterprise overhaul: in addition to /dev/shm/terminal-siblings,
// we ALSO write a `.claude/CLAUDE.md` directly INTO each worktree. Claude Code
// auto-loads this file on session start, so the agent sees its identity +
// siblings + budget the moment the user types their first message — no
// initial_prompt round-trip needed. The directory is added to `.git/info/exclude`
// per-worktree so it never shows in `git status` or `git diff`.
const WORKTREE_CLAUDE_RELDIR = ".claude"
const WORKTREE_CLAUDE_RELPATH = ".claude/CLAUDE.md"

function tmuxStatusBlurb(tmuxName: string): string {
  // Best-effort: capture last 30 lines of the tmux pane and pull a single-line
  // human summary out of it. Skips ANSI noise; falls back to an empty string
  // if the capture errors. Used both in WORKTREE_CLAUDE.md and the sibling MD.
  try {
    const r = spawnSync("tmux", ["-f", TMUX_CONF_PATH, "capture-pane", "-p", "-t", tmuxName, "-S", "-30"], { encoding: "utf8" })
    ...
    // Look for the most recent "interesting" line — one that's not the prompt
    // box decoration or a status footer. Take the last 80 chars.
    const last = stripped.slice(-5).reverse().find((l) => l.length > 4 && !/^[>❯⏵]/.test(l)) || ""
...
function buildSiblingMarkdown(forSession: Session, others: Session[]): string {
  ...
  for (const o of others) {
    ...
    const status = tmuxStatusBlurb(o.tmuxName)
    if (status) lines.push(`- Doing right now: ${status}`)
...
// Build the rich identity CLAUDE.md that lives at <worktree>/.claude/CLAUDE.md.
// Loaded automatically by Claude Code on session start. Re-rendered every 30s
// so the siblings list and "doing right now" lines stay current.
function buildWorktreeClaudeMd(self: Session, others: Session[], dbExtras?: { cost_usd?: number; cost_cap_usd?: number; wallclock_cap_h?: number }): string {
  const ageH = ((Date.now() - self.createdAt) / 3_600_000).toFixed(1)
  const lines: string[] = []
  lines.push(`# You are a terminal session on srv1197943`)
  lines.push("")
  lines.push(`This file is auto-generated by terminal-server every 30 seconds. It tells you exactly where you are, what your siblings are doing, and how to coordinate with them and with Dylan.`)
  lines.push("")
  lines.push(`## Your identity`)
  lines.push("")
  lines.push(`- **Session ID:** \`${self.id}\``)
  lines.push(`- **Title:** ${self.title}`)
  lines.push(`- **Worktree:** \`${self.worktreePath}\``)
  lines.push(`- **Branch:** \`${self.branch}\``)
  lines.push(`- **Tmux session:** \`${self.tmuxName}\` (you're inside it now)`)
  lines.push(`- **Started:** ${ageH}h ago`)
  if (dbExtras?.cost_cap_usd != null) {
    const used = (dbExtras.cost_usd ?? 0).toFixed(2)
    const cap = dbExtras.cost_cap_usd.toFixed(2)
    lines.push(`- **Spend so far:** $${used} of $${cap} cap (terminal-server will Ctrl-C twice if you hit the cap)`)
  }
  if (dbExtras?.wallclock_cap_h != null) {
    lines.push(`- **Wall-clock cap:** ${dbExtras.wallclock_cap_h}h since spawn (Telegram ping past that, no auto-stop)`)
  }
  lines.push("")
  lines.push(`## You can run for hours or days`)
  lines.push("")
  lines.push(`Your tmux session survives:`)
  lines.push(`- Browser tab close`)
  lines.push(`- Vercel deploy`)
  lines.push(`- Network drops`)
  lines.push(`- terminal-server restart (rehydrated on boot)`)
  lines.push(`- VPS reboot (\`destroy-unattached off\` in tmux.conf)`)
  lines.push("")
  lines.push(`Pace yourself. Long-running tasks are the whole point of this place — don't rush a multi-hour build.`)
  lines.push("")
  lines.push(`## Your siblings`)
  lines.push("")
  if (others.length === 0) {
    lines.push(`No other terminals are running right now. You're solo.`)
  } else {
    lines.push(`There are ${others.length} other terminal(s) on this VPS right now. Coordinate to avoid stepping on each other:`)
    lines.push("")
    for (const o of others) {
      const ageMin = Math.round((Date.now() - o.lastActivityAt) / 60_000)
      const status = tmuxStatusBlurb(o.tmuxName)
      lines.push(`- **${o.title}** (\`${o.id.slice(0, 8)}\`) on \`${o.branch}\` — last activity ${ageMin}m ago`)
      if (status) lines.push(`  - Doing right now: ${status}`)
    }
    lines.push("")
    lines.push(`The rich live status (more detail than this section) is at \`/dev/shm/terminal-siblings/${self.id}.md\` — re-read it every 30s if you're working on shared code.`)
  }
  lines.push("")
  lines.push(`## Reaching Dylan`)
  lines.push("")
  lines.push(`To ping Dylan via Telegram (blockers, approval requests, "I'm done"), write a single message to \`/dev/shm/notify-out/${self.id}.md\`. terminal-server picks it up within 30s and routes via \`dispatchNotification\`.`)
  lines.push("")
  lines.push(`Use sparingly. Dylan is a college student — don't fire one of these for routine progress, only for things he needs to act on.`)
  lines.push("")
  lines.push(`## Coordination etiquette`)
  lines.push("")
  lines.push(`- **Before touching shared code:** glance at the sibling list above. If a sibling lists the same file in their files_touched, commit your work first or pick a different file.`)
  lines.push(`- **Before opening a PR that touches the dashboard frontend:** make sure no sibling is mid-PR on the same files (\`gh pr list --state open\`).`)
  lines.push(`- **Before restarting a long-running service:** check sibling activity — restarting terminal-server, openclaw, or rtrvr-sender will affect anyone attached.`)
  lines.push("")
  lines.push(`---`)
  lines.push(`_Auto-regenerated by /opt/terminal-server every 30s. Edit terminal-server source to change this template._`)
  return lines.join("\n")
}

// One-shot: ensure the repo's shared .git/info/exclude carries an entry for
// the auto-generated worktree CLAUDE.md. Linked worktrees share this file
// with the main checkout (per-worktree info/exclude needs extensions.worktree-
// Config=true and isn't enabled here), so a single write handles every worktree
// the terminal-server creates from now on. Idempotent.
let excludePathPrimed = false
async function primeRepoExclude(anyWorktreePath: string): Promise<void> {
  if (excludePathPrimed) return
  try {
    const r = spawnSync("git", ["-C", anyWorktreePath, "rev-parse", "--git-common-dir"], { encoding: "utf8" })
    if (r.status !== 0) return
    const sharedGitDir = r.stdout.trim()
    const excludePath = join(sharedGitDir, "info", "exclude")
    const fs = await import("node:fs")
    // No existsSync→read TOCTOU: read with try/catch and treat ENOENT as empty.
    // Append-only write with O_APPEND avoids the read-modify-write race entirely
    // when this code runs concurrently across workers.
    let cur = ""
    try {
      cur = fs.readFileSync(excludePath, "utf8")
    } catch (err) {
      if ((err as NodeJS.ErrnoException).code !== "ENOENT") throw err
    }
    if (cur.includes("/.claude/CLAUDE.md")) {
      excludePathPrimed = true
      return
    }
    const sep = cur.endsWith("\n") || cur === "" ? "" : "\n"
    const payload = sep + "# Auto-generated by terminal-server (per-session identity briefing)\n/.claude/CLAUDE.md\n"
    // Append, not replace — race-safe across workers + idempotent (the next
    // call short-circuits because the substring is now present).
    fs.appendFileSync(excludePath, payload, { encoding: "utf8", flag: "a" })
    console.log("[terminal-server] primed shared .git/info/exclude with /.claude/CLAUDE.md")
    excludePathPrimed = true
  } catch (e) {
    console.warn("[terminal-server] could not prime .git/info/exclude:", (e as Error).message)
  }
}

async function writeWorktreeClaudeMd(self: Session, others: Session[]): Promise<void> {
  const dir = join(self.worktreePath, WORKTREE_CLAUDE_RELDIR)
  const path = join(self.worktreePath, WORKTREE_CLAUDE_RELPATH)
  try {
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true })
    await primeRepoExclude(self.worktreePath)
    let extras: { cost_usd?: number; cost_cap_usd?: number; wallclock_cap_h?: number } | undefined
    if (supa) {
      const { data } = await supa.from("terminal_sessions").select("cost_usd, cost_cap_usd, wallclock_cap_h").eq("id", self.id).maybeSingle()
      if (data) extras = data as typeof extras
    }
    const md = buildWorktreeClaudeMd(self, others, extras)
    const { writeFileSync } = await import("node:fs")
    writeFileSync(path, md, "utf8")
  } catch (e) {
    console.warn("[terminal-server] worktree CLAUDE.md write failed:", { id: safe(self.id), err: (e as Error).message })
  }
}

// Telegram ping primitive: per-session messages can be queued by the session
// itself by writing to /dev/shm/notify-out/<id>.md. This watcher drains the
// directory every 30s, dispatches via Telegram, and removes the file.
const NOTIFY_OUT_DIR = "/dev/shm/notify-out"

async function notifyOutWatcherTick(): Promise<void> {
  if (!existsSync(NOTIFY_OUT_DIR)) {
    try { mkdirSync(NOTIFY_OUT_DIR, { recursive: true }) } catch { /* */ }
    return
  }
  const fs = await import("node:fs")
  let entries: string[]
  try { entries = fs.readdirSync(NOTIFY_OUT_DIR) } catch { return }
  for (const name of entries) {
    if (!name.endsWith(".md")) continue
    const id = name.slice(0, -3)
    const s = sessions.get(id)
    if (!s) continue
    const filepath = join(NOTIFY_OUT_DIR, name)
    let body: string
    try { body = fs.readFileSync(filepath, "utf8").trim() } catch { continue }
    if (!body) {
      try { fs.unlinkSync(filepath) } catch { /* */ }
      continue
    }
    const text = `🤖 *${s.title}* (\`${s.id.slice(0, 8)}\`):\n\n${body}`
...
async function siblingWriterTick(): Promise<void> {
  ...
  for (const s of list) {
    ...
    // Also refresh the worktree-local .claude/CLAUDE.md so claude's session
    // picks up the latest sibling list and budget on its next CLAUDE.md
    // re-read (auto-compaction, /clear, etc).
    void writeWorktreeClaudeMd(s, others)
  }
...
function startWatchers(): void {
  ...
  setInterval(() => { void notifyOutWatcherTick() }, 30_000).unref()
  console.log("[terminal-server] watchers started: cost(60s) wallclock(5m) crash(30s) siblings(30s) notify-out(30s)")
}
